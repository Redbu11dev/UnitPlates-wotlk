...
function UPCoreFrameFadeOnUpdate(self, elapsed)
    ...
    if #UNITPLATES_FADEFRAMES == 0 then
...
function UPCoreFrameFade(frame, info)
    ...
    UnitPlatesFrameFadeFrame:SetScript('OnUpdate', UPCoreFrameFadeOnUpdate)
end
...
function UPCoreGetUnitIDFromName(name)
...
function UPCoreGetUnitIDFromGUID(guid)
	if not guid then return nil end
...
function UPCoreGetNameplateByGuid(guid)
	local frames = select("#", WorldFrame:GetChildren())
	local f
	for i = 1, frames do
		f = select(i, WorldFrame:GetChildren())
		if f.kui and (f.kui.guid == guid) then
			return f.kui
		end
	end
